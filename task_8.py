"""
Создать телефонный справочник с возможностью импорта и экспорта данных вформате .txt. Фамилия, имя, отчество, номер
...
def copy_data(file_name, file_name_new):
    ...
    res = read_file(file_name)
    res_copy = read_file(file_name_new)
    if search <= len(res):
        res_copy.append(res[search - 1])
        standart_write(file_name_new, res_copy)
    else:
        print("Некорректный номер строки")


def copy_data_all(file_name, file_name_new):
    res = read_file(file_name)
    standart_write(file_name_new, res)
...
def main():
    while True:
        ...
        if command == 'q':
            ...
            if not exists(file_name):
                print("Файл, который хотите скопировать, отсутствует, создайте файл")
                continue
            copy_data_all(file_name, file_name_new)
